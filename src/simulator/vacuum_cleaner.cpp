...
void VacuumCleaner::initializeSensor() {
    ...
    sensor_ = std::make_shared<BumperSensor>(sensor_id, name_, std::move(sensor_shape), position_, weak_from_this(), registry_);
...
Object::ObjectMap VacuumCleaner::evolve(float delta_t) {
    if (sensor_->sense()) {
        // A hit occured.
        // State (position) is reverted by Object. Here we just make a decision about what to do upon bump
        elapsedTimeOnStateSec_ = 0;
