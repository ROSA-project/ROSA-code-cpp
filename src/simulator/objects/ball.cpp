...
RigidPointBall::RigidPointBall(const ObjectId& oid,
                ...
                float acceleration,
                float velocity)
                : RigidPhysicalObject(oid, name, std::move(shape), position, owner_object, registry, acceleration, velocity) {
...
//     // That is, we're here because infinitesimal intersections had occurred in the previous cycle.
//     // TODO: assuming other objects are still, meaning that we have no change in momentum in other words we are not currently modelling bump of two moving objects

//     for (auto &in_in: latestIntersections_) {
//         if (!in_in->doesIntersect() || !in_in->isInfinitesimal()) {
//             continue;
//         }
//         // TODO: I'm taking the first bump, process and return! we can have multiple bumps at the same time resulting in a combined change.

//         // logger.Logger.add_line("processing a bump:")

//         // TODO: we won't have this method. just an oversimplification for now bump_point expected to be a numpy array with 3 elements x, y and z
//         auto& intersection_points = in_in->getIntersectionPoint();
